...
const BillRefValue = () => {
  ...
  const availableFields = [
    ...
    { id: "refno.", name: "Ref No." },
    { id: "type", name: "Credit/Debit" },
  ];
  ...
  const fetchFieldArrangement = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/getRefNoofthatbranch`,
        {
          params: {
            orgname: localStorage.getItem("orgname"),
            orgcode: localStorage.getItem("orgcode"),
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      for (const serverField of serverFields) {
        try {
          await axios.delete(`${API_BASE_URL}/deleteRefArrangement`, {
            data: {
              orgname: localStorage.getItem("orgname"),
              orgcode: localStorage.getItem("orgcode"),
        ...
        try {
          if (field.isCustom) {
            ...
            const response = await axios.post(
              `${API_BASE_URL}/storeRefArrangement`,
              {
                orgname: localStorage.getItem("orgname"),
                orgcode: localStorage.getItem("orgcode"),
            ...
            await axios.put(`${API_BASE_URL}/updateRefColumn`, {
              orgname: localStorage.getItem("orgname"),
              orgcode: localStorage.getItem("orgcode"),
              branchname: selectedBranch.label,
              branchcode: selectedBranch.value,
            ...
            // Add regular field
            await axios.post(`${API_BASE_URL}/storeRefArrangement`, {
              orgname: localStorage.getItem("orgname"),
              orgcode: localStorage.getItem("orgcode"),
              data: field.name,
              branchname: selectedBranch.label,
      ...
      <CCard className="global-card">
        <div style={{ margin: "10px 40px" }}>
          ...
          {selectedBranch && (
            ...
            >
              <h2 className="textType2-JobValue">Customize Format: Ref No.</h2>
            </div>
          )}
                ...
                <div className="mt-2 mb-3">
                  {availableFields.map((field) => (
                    <button
                      ...
                      style={{
                        backgroundColor: fields.some((f) => f.id === field.id)
                          ? "#e0e0e0"
                          : "#f0f0ff",
                        border: "1px solid #ccc",
                        borderRadius: "4px",
                        cursor: fields.some((f) => f.id === field.id)
                          ? "not-allowed"
                          : "pointer",
                      }}
                      onClick={() => {
