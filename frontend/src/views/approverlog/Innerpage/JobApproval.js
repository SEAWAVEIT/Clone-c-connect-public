...
const JobApproval = () => {
  ...
  const handleApprove = (jobData) => {
    // Handle job approval
    handleApproveJob(jobData);
    console.log("Approving job:", jobData);
  };

  const handleReject = (jobData) => {
    // Handle job rejection
    handleRejectJob(jobData);
    console.log("Rejecting job:", jobData);
  };
  ...
  const fetchApprovers = async () => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/getapproverofJobs`,
        {
          params: {
  ...
  const fetchLatestJobs = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/fetchlatestjob`, {
        params: {
          orgname: localStorage.getItem("orgname"),
          orgcode: localStorage.getItem("orgcode"),
  ...
  async function checker() {
    try {
      const response = await axios.get(`${API_BASE_URL}/getapprovedJob`, {
        params: {
          orgname: localStorage.getItem("orgname"),
          orgcode: localStorage.getItem("orgcode"),
          uniquevalue: uniqueValue,
  ...
  const handleApproveJob = async () => {
    try {
      await axios.put(`${API_BASE_URL}/approveImpJob`, {
      ...
      // );
      const updatedLatestJob = latestJobs.map((job) =>
      ...
      setalljobsimp(updatedLatestJob);
      refreshData();
    } catch (error) {
  ...
  const exportToExcel = async () => {
    ...
    link.download = "ImpApproval.xlsx";
    link.click();
  };
  ...
  const filterImpJobs = () => {
    const filtered = latestJobs.filter((job) => {
      ...
      const matchesImporter = job.importername
        .toLowerCase()
  ...
  const handleRestoreJob = async (job) => {
    try {
      await axios.put(`${API_BASE_URL}/restoreImpJob`, {
        orgname: localStorage.getItem("orgname"),
        orgcode: localStorage.getItem("orgcode"),
        jobnumber: job,
      });
                  ...
                  approvalName?.some(
                    ...
                    currentItems
                      ...
                      .map((item, index) => {
                        ...
                        const isApproved = alljobsimp?.some(
            ...
            {selectedJob && (
              <>
                <div className="impapprovalmodal">
                  ...
                  <input
                    type="text"
                    value={selectedJob.importername}
                    readOnly
                  />

                  <label>Branch</label>
                  <input type="text" value={selectedJob.branchname} readOnly />
                </div>
