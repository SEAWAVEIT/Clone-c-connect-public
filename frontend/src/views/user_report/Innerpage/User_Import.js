...
const User_Import = ({
  ...
  const fetchAllData = async (branch = "All", org = "All") => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/getAllRowsofUsername`,
        {
          params: {
  ...
  const getPieChartData = () => {
    ...
    const chartData = relevantWorkflows.map((item) => {
      ...
      // Calculate percentage - explicitly return 0 if either count is 0
      return totalJobs > 0 && completedCount > 0
        ? parseFloat(((completedCount / totalJobs) * 100).toFixed(2))
        : 0;
    });
            ...
            {allData.rowshaiye &&
              allData.rowshaiye.map((item, index) => {
                const completedRowsForItem = allData.completedRows.filter(
                  (row) =>
                    row.tatimpcolumn === item.workflowName &&
                    (selectedBranchofUser === "All" || 
                ...
                // Use the same totalCount calculation for both table and chart
                const totalCount = 
                  selectedBranchofUser === "All"
                    ? allData.totalJobs.length
                    : allData.totalJobs.filter(
