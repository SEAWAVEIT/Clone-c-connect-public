...
import styles from "./css/impDelayedJob.module.css";
...
function impDelayedJobs() {
  ...
  const fetchImporters = async () => {
    try {
      ...
      // console.log(
      //   "Importer: ",
      //   response.data.map((importer) => importer.clientname)
      // );
    } catch (error) {
  ...
  const getMilestones = async () => {
    try {
      ...
      const workflowLobNames = ["Air IMPORT", "Sea IMPORT"];
      ...
      // console.log("Milestone: ", filtered);
  ...
  const getDelayedJobs = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/impdelayedjobs`, {
        params: {
          orgname: localStorage.getItem("orgname"),
          orgcode: localStorage.getItem("orgcode"),
  ...
  const getDelayedMilestone = async () => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/impdelayedmilestones`,
        {
          params: {
            orgname: localStorage.getItem("orgname"),
            orgcode: localStorage.getItem("orgcode"),
  ...
  const getDelayedJobsDetails = async (jobno) => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/impdelayedjobdetails`,
        {
          params: {
            jobnumber: jobno,
            orgname: localStorage.getItem("orgname"),
  ...
  const SortedJobs = delayedJobs.filter((row) => {
    ...
    return (
      ...
      (!selectedStartDate ||
        rowDate >= moment(selectedStartDate).startOf("day")) && // Check if rowDate is after or equal to startDate
      (!selectedEndDate || rowDate <= moment(selectedStartDate).endOf("day")) && // Check if rowDate is before or equal to endDate      !selectedImporter ||
      row.ImporterName.toLowerCase().includes(
        (selectedImporter || "").toLowerCase()
      ) &&
      (!searchedJobNo ||
      ...
      <div className={styles.container}>
        <div className={styles.firstRow}>
          ...
          <div className={styles.importBox}>
            <div className={styles.importTitle}>{activeTab}</div>
          </div>
        </div>
