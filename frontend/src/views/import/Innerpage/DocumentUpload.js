...
const DocumentUpload = () => {
  ...
  const filelocation = (filename) => {
    ...
    return `${API_BASE_URL}/uploads/import/${jobnumber}/${filename}`;
  };
  ...
  const originalFileName = uploadedFiles;
    ...
    if (updatedFileName.split("_")[0] != originalFileName.split("_")[0]) {
      ...
      try {
        const response = await axios.put(`${API_BASE_URL}/rename-file`, {
          ...
          updatedFileName,
          type: "import",
          jobnumber: String(jobNumber).replace(/\//g, "-"),
        });
        ...
        if (
          ...
          setUploadedFiles((prev) =>
            prev.map((file, i) =>
              i === index
                ? { ...file, filename: updatedFileName.filename }
                : file
            )
          );
      ...
      setUploadedFiles((prev) =>
        prev.map((file, i) =>
          i === index ? { ...file, filename: updatedFileName.filename } : file
        )
      );
      setEditingFileIndex(null);
      toast.success("File renamed successfully.");
  ...
  return (
    ...
    >
                  ...
                  >
                    {editingFileIndex === index ? (
                      <input
                        ...
                        onClick={(e) => e.stopPropagation()} // Prevents row click while renaming
                        onChange={(e) =>
                      ...
                      <span
                        onDoubleClick={(e) => {
                          ...
                          handleEditFile(index); // Enters rename mode
                        }}
                      >
                        {file.filename}
                      </span>
      ...
      {/* <InputPopup title={"File Upload"} setCurrentPopup={setCurrentPopup} fields={}/> */}
