...
const Collection = () => {
  ...
  const handleCtrlB = (e) => {
    if (e.ctrlKey && e.key === "b") {
      ...
      if (parentRef.current) {
        ...
        setParentLeft(rect.left + rect.width / 2);
      }
    }
  };
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("keydown", handleCtrlB);
  }, []);
  const contactFields = [
    {
      id: "remark",
  ...
  useEffect(() => {
    ...
    checkToken();
    getCreditDays(); // refreshes creditdays after refreshing from the browser
  }, [navigate]);
  ...
  const getCreditDays = async () => {
    try {
      ...
      const clientname = sessionStorage.getItem("importername");
  ...
  const fetchCollections = async () => {
    try {
      ...
      // Update each collection with dynamically calculated TimeDelay
      const updatedCollections = response.data.map((collection) => {
        const followUpDate = new Date(collection.FollowUp1);
        if (isNaN(followUpDate.getTime())) return collection; // Skip if invalid

        const timeDiff = today.getTime() - followUpDate.getTime();
        const calculatedDelay = Math.floor(timeDiff / (1000 * 60 * 60 * 24));

        return {
          ...collection,
          TimeDelay: calculatedDelay >= 0 ? calculatedDelay : 0, // Ensure no negatives
        };
      });
    ...
    >
                ...
                {collections.map((collection, index) => (
                  ...
                  >
                    ...
                    <td
                      ...
                      style={{ minWidth: "100px" }}
                    >
                      {new Intl.NumberFormat("en-IN").format(collection.amount)}
                    </td>
                    <td
                      className="td-accounts py-1 rounded-lg"
                      style={{ minWidth: "100px" }}
                    ...
                    >
                      {/* <select
                        name="AssignTo"
                        className=" form-select-imp-create"
                        value={collection.AssignTo}
                        style={{
                          padding: "2px",
                          border: "1px solif #C3C3C5 !important",
                        }}
                        onChange={(e) =>
                          handleAssigneeChange(collection.id, e.target.value)
                        }
                      >
                        <option
                          style={{ padding: "2px", borderColor: "c7c7c7" }}
                          value=""
                        >
                          Select Employee
                        </option>
      ...
      >
        <CModalHeader onClose={() => setShowEditModal(false)}>
          <CModalTitle id="LiveDemoExampleLabel">Edit Bill Details</CModalTitle>
        </CModalHeader>
        <CModalBody>
          {selectedCollection && (
            <div>
              ...
              <div style={{ display: "flex" }}>
                <label htmlFor="date" className="impcollection-txt-field">
                  Date
                </label>
                <div id="date" type="date" className="text-field-1" name="date">
              ...
              {/* Bill No. */}
              <div style={{ display: "flex" }}>
                <label htmlFor="billNo" className="impcollection-txt-field">
                  Bill No.
                </label>
                <label
                  id="billNo"
                ...
                >
                  {" "}
                  {selectedCollection.billNo}{" "}
                </label>
              </div>

              {/* Branch Name */}
              <div style={{ display: "flex" }}>
                <label htmlFor="branchname" className="impcollection-txt-field">
                  Branch Name
                </label>
                <div
                  id="branchname"
                  ...
                  name="branchname"
                >
                  {selectedCollection.branchnameoforg}
                </div>
              </div>

              {/* Amount */}
              <div>
                <label htmlFor="amount" className="impcollection-txt-field">
                  Amount
                </label>
                <input
                  id="amount"
              ...
              {/* Tax */}
              <div>
                <label htmlFor="Tax" className="impcollection-txt-field">
                  Tax
                </label>
                <input
                  id="Tax"
              ...
              {/* Grand Total */}
              <div>
                <label htmlFor="grandTotal" className="impcollection-txt-field">
                  Grand Total
                </label>
                <input
                  id="grandTotal"
                  type="number"
                  className="text-field-1"
                  value={selectedCollection.grandTotal || ""}
                  readOnly
                />
              </div>
            </div>
      ...
      >
        ...
        <CModalBody>
          <p>
            Are you sure you want to delete Bill No {collectionToDelete?.billNo}
            ?
          </p>
          <CFormInput
            value={remark}
